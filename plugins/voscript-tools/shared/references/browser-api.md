...
### Whether the title carries the accession is a per-vendor fact — and it changes

**It is not safe to assume either way.** BXLink used to have a stable product title and the vendor
changed it; every BXLink script running against an open case was silently resolving no window
until the helpers were switched over. Concentriq is the opposite case: its title *never* carries
the case number, so the fixed-fragment lookup is the correct choice there.

State of the shipped integrations, as of 2026-09-03:

| System | Fixed title fragment | Open-case scripts use |
|---|---|---|
| BXLink | `Lumea+`, `BxLink` | `ByRegex` (`_BXLink.CaseWindowPattern`) — worklist title only, accession once a case is open |
| Fusion | `Worklist \| Techcyte` | `ByRegex` — the fragment is explicitly the worklist |
| Halo | — | `ByRegex` |
| PathPresenter | `Dashboard`, `Cases List`, … | `ByRegex` |
| Concentriq | — | **fixed, and correctly so** — the title never carries the case number |
| AISight, Corista, Fuji, PathFlow | — | **fixed only** — never audited, and `_Corista` / `_Fuji` / `_PathFlow` / `_AISight` do not even define a `ByRegex` helper |

The last row is the risk. Before writing anything new in those namespaces, check what the window
title actually is on an open case: read it off the browser, or log
`WindowTools.Instance.GetWindowText(...)` once. If it carries the accession, the namespace needs a
`FindCurrentTitlePageByRegex` added to its library — copy the one in `_PathPresenter` verbatim —
and its open-case scripts switched over. Put the accession pattern in a `const` on the library
rather than repeating the literal in every command.
...
The `EnsureWindow` pattern wraps these: reuse the window for this surface if it exists, otherwise
open one, then confirm the title carries the case number before moving on. That keeps one window
per surface instead of one per case.

(The reference implementation was `_Flexlis.cs`, which is not present in the current instance —
see the note in `script-catalog.md`. Build it from the helpers above if you have no example.)
...
- **Never address a control by Chrome's missing-image placeholder text.** Two shipped scripts
  click elements named `"To get missing image descriptions, open the context menu."` (BXLink) and
  `"PP Logo. To get missing image descriptions, open the context menu."` (PathPresenter). That
  string is *generated by the browser* for an image with no alt text — it is not the vendor's
  accessible name. It changes with Chrome version and with the user's browser locale, so a script
  built on it breaks on a browser update or on any non-English workstation, with a
  "element not found" that points nowhere near the cause. If a control's only name looks like
  browser chrome or an instruction to the user, treat it as unnamed and anchor off a named
  neighbour instead.
...
- **`IsElementVisible` only tests presence.** In Halo the "Add Assay" button stays in the
  ...
  **This one is still shipping.** `BXLink.NextCase` opens with
  `if (_Browser.IsElementVisible(manager, "Close Encounter"))` and branches on the result, so the
  branch is chosen by whether the element exists in the tree rather than whether it is on screen.
  Do not copy that shape from the cache.

- **Dispose the `BrowserManager`, not just the controller.** `Corista.NextCase` does
  `var m_SlideViewer = new BrowserManager(titlePage);` with only the controller in a `using`, so
  the manager is never disposed. Put both in `using var`, or dispose both in the `finally`. The
  leak is silent until PRO starts crashing.
